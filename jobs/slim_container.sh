...
### ======== PATHS ========
export CODE_DIR="${CODE_DIR:-/home/groups/roxanad/sonnet/compressed-perception}"
...
3. FIRST TIME ONLY - Inside the container, create venv and install PyTorch:
   ...
   # Using python -m pip ensures we're using the venv pip
   ...
   python -m pip install --no-cache-dir peft
   python -m pip install --no-cache-dir json5
   python -m pip install --no-cache-dir google-cloud-aiplatform
   ...
   python -m pip install --no-cache-dir datasets
   python -m pip install --no-cache-dir matplotlib
   python -m pip install --no-cache-dir seaborn

   # If you have requirements.txt file, install packages one by one:
...
5. Running your SAFRAN scripts:
   # Inside the container with venv activated:
   python scripts/calculate_safran_scores.py
   python scripts/student_distillation_two_models.py
   # etc.
