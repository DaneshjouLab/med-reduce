...
from torch.optim.lr_scheduler import _LRScheduler, LambdaLR, StepLR, ReduceLROnPlateau
...
def _build_optimizer(cfg, params) -> Optimizer:
    opt_cfg = getattr(cfg, "train").get("optimizer", {}) if hasattr(cfg, "train") else {}
    name = str(opt_cfg.get("name", "adamw")).lower()
    lr = float(opt_cfg.get("lr", 1e-4))
    wd = float(opt_cfg.get("weight_decay", 0.05))

    if isinstance(params, torch.nn.Module) or (hasattr(params, "__iter__") and hasattr(next(iter(params)), "ndim")):
...
def _build_scheduler(cfg, optimizer: Optimizer) -> Tuple[_LRScheduler | None, Dict[str, Any]]:
    sch_cfg = getattr(cfg, "train").get("scheduler", {}) if hasattr(cfg, "train") else {}
    ...
    if name in ("cosine", "cosineanneal", "cosine_anneal"):
        ...
        base_lr = float(getattr(cfg.train.get("optimizer", {}), "lr", 1e-4) if hasattr(cfg, "train") else 1e-4)
        ...
        return scheduler, {"by": "epoch"}
    elif name in ("plateau", "reduceonplateau"):
...
def step_scheduler(scheduler, meta: Dict[str, Any], epoch: int, val_metric: float | None = None):
    """
...
def make_optimizer_and_scheduler(cfg, params) -> Tuple[Optimizer, Tuple[_LRScheduler | None, Dict[str, Any]]]:
    """
